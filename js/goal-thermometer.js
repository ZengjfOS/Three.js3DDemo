...
var animationTime = 300;//in milliseconds
...
function animateThermometer(){
	...
	$({tipAmount:flagTime}).animate({tipAmount: currentAmount}, {
		duration:animationTime,
		step:function(){
			tooltipTxt.html(commaSeparateNumber(this.tipAmount));
			flagTime = currentAmount;
		}
	});
...
function startAnim(){
	...
	//图片动画
	mercuryId.animate({height:mercuryHeight +1, top:newMercuryTop }, ajaxAnimationTime);
	//文字框动画
	tooltipId.animate({top:newMercuryTop - tooltipOffset}, {duration:ajaxAnimationTime});
	
	var tooltipTxt = $("#therm-tooltip .tip-middle p");
	//文字动画
	//change the tooltip number as it moves
	$({tipAmount: flagTime}).animate({tipAmount: currentAmount}, {
		duration:ajaxAnimationTime,
		step:function(){
			tooltipTxt.html(commaSeparateNumber(this.tipAmount));
			flagTime = currentAmount;
		}
